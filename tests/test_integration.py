...
class TestEndToEndProfiling:
    ...
    def test_profile_mixed_types_dataframe(self, spark_session):
        ...
        assert profile["overview"]["memory_usage"] > 0
    ...
    def test_profile_large_dataframe_with_sampling(self, spark_session):
        ...
        assert profile["sampling"]["sample_fraction"] < 1.0
        assert profile["sampling"]["quality_score"] > 0.8
        
        # Verify profile still contains valid statistics
        assert profile["overview"]["total_rows"] == 15_000_000
    ...
    def test_profile_with_custom_sampling_config(self, spark_session):
        ...
        assert profile["sampling"]["config"]["confidence_level"] == 0.99
    ...
    def test_profile_output_formats(self, spark_session):
        ...
        dict_output = profiler.get_profile("dict")
        assert isinstance(dict_output, dict)
        assert "overview" in dict_output
        assert "columns" in dict_output
        
        # Test JSON format
        json_output = profiler.get_profile("json")
        assert isinstance(json_output, str)
        parsed = json.loads(json_output)
        assert parsed == dict_output
        
        # Test summary format
        summary_output = profiler.get_profile("summary")
    ...
    def test_profile_edge_cases(self, spark_session):
        ...
        null_df = spark_session.createDataFrame(null_data, ["col1", "col2", "col3"])
    ...
    def test_column_selection_profiling(self, spark_session):
        ...
        profiler = DataFrameProfiler(df)
        profile = profiler.profile(columns=["id", "category", "metric1"])
    ...
    def test_error_handling_integration(self, spark_session):
        ...
        with pytest.raises(ValueError):
            profiler.profile(columns=["non_existent_column"])
        
        # Test invalid output format
        with pytest.raises(ValueError):
            profiler.get_profile("invalid_format")
