...
class SamplingMetadata:
    ...
    def speedup_estimate(self) -> float:
        ...
        return 1.0 / self.reduction_ratio if self.reduction_ratio > 0 else 1.0
...
class RandomSamplingStrategy(SamplingStrategy):
    ...
    def _calculate_sampling_fraction(self, original_size: int, config: SamplingConfig) -> float:
        ...
        target_size = min(config.max_sample_size, max(config.min_sample_size, int(original_size * 0.01)))
