...
class BatchStatisticsComputer:
    ...
    def _compute_column_stats_optimized(self, column_name: str, column_type) -> Dict[str, Any]:
        """
        ...
        from pyspark.sql.functions import (
            count, when, isnan, isnull, min as spark_min, max as spark_max,
            mean, stddev, expr, length, approx_count_distinct
        )
...
def optimize_dataframe_for_profiling(df: DataFrame, sample_fraction: Optional[float] = None) -> DataFrame:
    ...
    if row_count < 1000 and current_partitions > 1:
        ...
        optimized_df = optimized_df.repartition(target_partitions)
